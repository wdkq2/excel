...
Send a POST request to `/extract` with one or more files (multipart/form-data).
The API reads each upload, extracts the four summary values and generates an
Excel workbook that mirrors the layout shown in the documentation. The
workbook is returned directly as the response so your browser will download
`results.xlsx`.
...
To test the API manually and download the resulting workbook:
